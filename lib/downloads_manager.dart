...
class DownloadsPage extends StatefulWidget with WidgetsBindingObserver {
  ...
  final Iterable<TaskInfo> documents;

  DownloadsPage({Key? key, required this.title, required this.documents, required this.platform}) : super(key: key);
...
class _DownloadsPageState extends State<DownloadsPage> {
      ...
      if (_tasks != null && _tasks.isNotEmpty) {
        ...
        if (task != null) {
          /*if(task.status == DownloadTaskStatus.complete){
            LocalAdhkarDataSourceImpl.updatePostDownloadInfo(task.taskId, task.type, downloadPath)
          }*/
          setState(() {
            task.status = status;
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        ...
        /*actions: [
          IconButton(
              icon: Icon(Icons.cloud_download), onPressed: _requestDownloadAll)
        ],*/
      ),
      body: Builder(
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Text(
                  S.of(context).grant_storage_permission,
  ...
  void _requestDownloadAll() {
    _items.map((e) {
      if (e.task.link != null) _requestDownload(e.task);
    });
  }
  ...
  // Future _cancelDownload(TaskInfo task) async {
  ...
  Future<Null> _prepare() async {
    ...
    _tasks.addAll(widget.documents.map((document) => TaskInfo(type: document.type, postId: document.postId, name: document.name, link: document.link)));
    ...
    //_tasks.addAll(_images.map((image) => TaskInfo(name: image['name'], link: image['link'])));
    ...
    //_tasks.addAll(_videos.map((video) => TaskInfo(name: video['name'], link: video['link'])));
...
class DownloadItem extends StatelessWidget {
              ...
              child: Row(
                ...
                children: <Widget>[
                  ...
                  Padding(
                    ...
                    child: _buildActionForTask(data.task, context),
                  ),
                ],
              ),
            ),
  ...
  Widget? _buildActionForTask(TaskInfo task, BuildContext context) {
    if (task.status == DownloadTaskStatus.undefined) {
      ...
      return Text(S.of(context).canceled, style: TextStyle(color: Colors.red));
    } else if (task.status == DownloadTaskStatus.failed) {
      return Row(
        mainAxisSize: MainAxisSize.min,
        mainAxisAlignment: MainAxisAlignment.end,
        children: [
          Text(S.of(context).failed, style: TextStyle(color: Colors.red)),
          RawMaterialButton(
            onPressed: () {
              onAtionClick(task);
            },
      ...
      return Text(S.of(context).pending,
          style: TextStyle(
            color: Colors.orange,
          ));
    } else {
